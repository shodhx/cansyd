...
def hero(path):
    ...
    ax.set_xlim(0, 120); ax.set_ylim(0, 34); ax.axis("off")
    ...
    ax.text(6, 22.5, "CNSD", fontsize=54, fontweight="bold", color=INK,
            va="center", zorder=3)
    ax.text(6.4, 12.8, "Causal Neuro-Symbolic Diagnosis", fontsize=15.5,
            color=BLUE, fontweight="bold", va="center")
    ax.text(6.6, 7.0, "Verify \u2014 don't just predict \u2014 bearing faults",
            fontsize=11, color=SLATE, va="center")
    ax.text(6.6, 3.2, "under operating-condition shift.",
            fontsize=11, color=SLATE, va="center")
    ...
    xa, xb = 74, 116
    ax.plot(np.linspace(xa, xb, x.size), 26 + 3.2 * burst / np.abs(burst).max(),
            color=SLATE, lw=0.8, zorder=2)
    ...
    ax.text(xa, 12.2, "envelope spectrum  →  characteristic-frequency comb",
    ...
    for name, col in chips:
        w = 0.44 * len(name) + 2.4
        ax.add_patch(FancyBboxPatch((cx, 2.4), w, 4.0,
                     boxstyle="round,pad=0.15,rounding_size=1.4",
                     fc="white", ec=col, lw=1.5, zorder=3))
        ax.text(cx + w / 2, 4.4, name, ha="center", va="center", fontsize=6,
...
def architecture(path):
    ...
    ax.set_xlim(0, 116); ax.set_ylim(0, 60); ax.axis("off")

    def box(x, y, w, h, title, sub, accent, fc="white"):
        ax.add_patch(FancyBboxPatch((x, y), w, h,
                     boxstyle="round,pad=0.4,rounding_size=1.6",
                     fc=fc, ec=accent, lw=2.0, zorder=3))
        ax.add_patch(plt.Rectangle((x + 0.6, y + 0.6), 1.1, h - 1.2, fc=accent,
                     ec="none", zorder=4))
        ax.text(x + w / 2 + 0.6, y + h - 3.4, title, ha="center", va="top",
                fontsize=9.5, fontweight="bold", color=INK, zorder=5)
        ax.text(x + w / 2 + 0.6, y + h - 8.2, sub, ha="center", va="top",
                fontsize=7.2, color=SLATE, linespacing=1.3, zorder=5)

    def arrow(x1, y1, x2, y2, color=SLATE, rad=0.0, lw=2.0, ls="-"):
        ax.add_patch(FancyArrowPatch((x1, y1), (x2, y2), arrowstyle="-|>",
                     mutation_scale=15, lw=lw, color=color, zorder=2, ls=ls,
                     connectionstyle=f"arc3,rad={rad}"))

    box(1, 22, 16, 13, "Vibration\nwindow", "raw signal\nx  \u2208  \u211d^L", SLATE, fc="#eef2f7")
    box(21, 21, 19, 15, "1 · Perception", "1-D CNN / S-JEPA\nproposes fault \u0177\n+ confidence c", VIOL)
    box(46, 39, 24, 15, "2 · Symbolic", "envelope-spectrum check\nCONFIRMED / CONFLICT /\nINCONCLUSIVE + root cause", TEAL)
    box(46, 21.5, 24, 14, "3 · Causal (Rung 2)", "do(Z) on corrected SCM\ninterventional warrant\n+ refutation suite", BLUE)
    box(46, 3.5, 24, 14, "3B · Counterfactual", "invertible SCM, RMS\nRung-3 stability\nunder do(Z:=z\u2032)", AMBER)
    ...
    ax.text(58, 58, "propose  →  verify  →  decide", ha="center", fontsize=10,
...
def results(path):
    ...
    phys = [d[1] for d in data]; ens = [d[2] for d in data]
    b1 = ax.bar(x - w/2, phys, w, label="Physics (CNSD)", color=BLUE, zorder=3)
    b2 = ax.bar(x + w/2, ens,  w, label="Deep ensemble", color="#cbd5e1", zorder=3)
    for i, d in enumerate(data):
        ax.text(i, max(phys[i], ens[i]) + 0.03, d[3], ha="center", fontsize=8,
                color=(GREEN if "n.s." not in d[3] else SLATE), fontweight="bold")
    ...
    ax.set_title("Physics verification vs. the strongest uncertainty baseline",
    ...
    ax.annotate("physics wins under shift", (0.5, 0.56), fontsize=8.5, color=GREEN,
                ha="center", style="italic")
    ax.annotate("null control:\nsaturated regime", (2, 0.30), fontsize=8, color=SLATE,
                ha="center", style="italic")
    fig.savefig(path, facecolor="white")
    plt.close(fig)
