...
```python
import cansyd
from cansyd import Dataset, PhysicsConfig, DiagnosisReport, CANSYD  # all resolve, no TensorFlow
import cansyd.causal, cansyd.physics, cansyd.config, cansyd.scm          # import cleanly
```
...
The library lives in `cansyd/`, organized one concern per package:

- `cansyd/perception` — Layer 1: 1D CNN + S-JEPA (requires `perception` extra)
- `cansyd/symbolic`, `cansyd/physics` — Layer 2: physics verification and providers
- `cansyd/causal`, `cansyd/scm` — Layer 3: Pearl Rung-2 intervention + refutation
- `cansyd/counterfactual` — Layer 3B: Rung-3 (requires `counterfactual` extra)
- `cansyd/consensus` — Layer 4: verdict + confidence → actionable status
- `cansyd/datasets`, `cansyd/diagnosis` — data contract and pipeline orchestration
...
overridden with the `CANSYD_DATA_CWRU`, `CANSYD_DATA_PU`, and `CANSYD_DATA_SEU`
...
- Open a PR and request review from a maintainer. Tag **both** maintainers for
  ...
  (`cansyd/__init__.py`, `cansyd/diagnosis/system.py`).
...
CANSYD is a research framework, and the results it produces go into a paper. We
