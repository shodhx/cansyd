...
class BearingProvider(PhysicsProvider):
    ...
    def evidence(self, signal, condition):
        rpm = self.cond_to_rpm.get(int(condition),
                                   next(iter(self.cond_to_rpm.values()), 1797))
        ev = fault_frequency_evidence(signal, rpm, fs=self.fs, bearing=self.bearing)
        return {
            'family_strength': {'Outer Race': ev['BPFO'],
                                'Inner Race': ev['BPFI'],
                                'Ball': ev['BSF']},
    ...
    def root_cause(self, family, evidence):
        ...
        if family == 'Normal':
            statement = 'No localized bearing defect; vibration is consistent with a healthy bearing.'
        elif freq_hz is not None:
            statement = (f'Defect on the {comp} ({family}), evidenced by a peak at the '
        ...
        return {'component': comp, 'fault_type': family, 'mechanism': mechanism,
                'evidence_frequency': freq_name, 'evidence_frequency_hz': freq_hz,
