"""
cross_rig.py - The cross-rig credibility experiment (CWRU -> Paderborn).
...
model learned 'bearing faults' or merely 'EDM notches on this one rig'. It is
the single highest-leverage honesty test in the project.
...
def run_cross_rig(cnn, rule_engine, damage='natural'):
    ...
    from data.paderborn_loader import load_paderborn_test, BEARING_6203
    from core.envelope import characteristic_frequencies
    ...
    print('\n[Interpretation] CWRU uses 6205 + EDM notches; Paderborn natural '
          ...
          'it bounds what the CNN actually generalises. Report this number '
          'honestly - it is the cross-rig result, success or failure.')
    return {'transfer_accuracy': acc, 'n_test': int(len(X_pu)),
            'damage': damage}
